...
namespace GesTienda
{
    public partial class PedidosPorCliente : System.Web.UI.Page
    {
        ...
        protected void grdClientes_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string StrComandoSql = "SELECT PEDIDO.IdPedido,FecPed,SerPed,CobPed," + "SUM(CanDet*PreDet-CanDet*PreDet*DtoDet/100) AS Total " 
                ...
                try
                {
                    ...
                    if (reader.HasRows)
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            StrResultado += "<div style='display:table-cell; text-align: center'>" + reader.GetValue(0) + "</div>";
                            StrResultado += "<div style='display:table-cell'>" + string.Format("{0:d}", reader.GetValue(1)) + "</div>";
